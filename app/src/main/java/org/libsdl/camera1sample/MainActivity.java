package org.libsdl.camera1sample;
...
//        LogUtils.d("MainActivity_log", "onResume: " + sound);
    ...
    @OnClick({R.id.openCameraBtn, R.id.initCamera, R.id.swichCamera})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.initCamera:
                initCamera(mBackCameraId);
                break;
            case R.id.swichCamera:
                /**
                 * 实际需求经常要求 APP 能够支持前后置摄像头的切换，所以这里我们也介绍下如何实现前后置摄像头的切换。
                 * 大部分情况下我们在切换前后置摄像头的时候，都会直接复用同一个 Surface，
                 * 所以我们会在 surfaceChanged() 的时候把 Surface 保存下来
                 *
                 *  切换摄像头流程：
                 * 1.停止预览
...
//        SurfaceHolder previewSurface = mSurfaceHolder;
